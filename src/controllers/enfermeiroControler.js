...
class LivroController{
    ...
    static async listarEnfermeiros (req,res){
        ...
        res.status(200).json(ListaE);}
            ...
            res.status(500).json({message:`${erro.message} - Falha ao Mostrar Enfermeiros Disponiveis`});
        }}
    ...
    static async listarEnfermeirosPorId (req,res){
        ...
        res.status(200).json(LivroE);}
            ...
            res.status(500).json({message:`${erro.message} - Falha ao mostrar Enfermeiro`});
        }
    }
    ...
    static async cadastrarEnfermeiro (req,res){
        try{
            ...
            res.status(201).json({message: "Criado com sucesso!", Enfermeiros: novoLivro})          // manejo de erros e sucessos
        }catch(erro){
    ...
    static async atualizarEnfermeiro (req,res){
        ...
        res.status(200).json({message: "Enfermeiro(a) atualizado!"});}
            ...
            res.status(500).json({message:`${erro.message} - Falha na atualização das informações do Enfermeiro(a)`});
        }
    }
    ...
    static async deletarEnfermeiro (req,res){
        ...
        res.status(200).json({message: "Enfermeiro Deletado!"});}
            ...
            res.status(500).json({message:`${erro.message} - Falha na remoção do Enfermeiro`});
        }
    }
